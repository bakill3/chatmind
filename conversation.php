...
// profile badge content (optional)
$profileSummaryHtml = '';
if ($has_profile) {
    ...
    <div class="card-body">
      ...
      <form id="ai-reply-form" method="POST" action="generate.php" class="composer">
        <div class="row g-3 align-items-center">
          ...
          <div class="col-12 col-xl-6">
            <div class="row g-3 align-items-end">
              ...
              <!-- NEW: Model selector -->
              <div class="col-12 col-md-6">
                <label for="model_override" class="form-label mb-1 small text-muted">Modelo</label>
                <select name="model_override" id="model_override" class="form-select form-select-sm">
                  <option value="">Automático (usa “Modo rápido”)</option>
                  <option value="<?= htmlspecialchars(defined('QUICK_MODEL_ALIAS') ? QUICK_MODEL_ALIAS : 'chatmind-mini') ?>">
                    Mini — <?= htmlspecialchars(defined('QUICK_MODEL_ALIAS') ? QUICK_MODEL_ALIAS : 'chatmind-mini') ?>
                  </option>
                  <option value="<?= htmlspecialchars(defined('LLAMA_MODEL_ID') ? LLAMA_MODEL_ID : 'chatmind') ?>">
                    Principal — <?= htmlspecialchars(defined('LLAMA_MODEL_ID') ? LLAMA_MODEL_ID : 'chatmind') ?>
                  </option>
                </select>
                <div class="form-text">Se escolheres um modelo aqui, ignora o “Modo rápido”.</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="form-check form-switch mt-2">
          <input class="form-check-input" type="checkbox" id="quick_mode" name="quick" value="1" <?= (defined('QUICK_MODE_DEFAULT') && QUICK_MODE_DEFAULT) ? 'checked' : '' ?> />
          <label class="form-check-label small" for="quick_mode">Modo rápido (usa modelo pequeno quando disponível)</label>
        </div>

        <input type="hidden" name="upload_id" value="<?= $upload_id ?>">
        <input type="hidden" name="contact_name" id="contact_name" value="<?= htmlspecialchars($other_name) ?>">
      ...
      <div id="ai-reply-box" class="mt-3" style="display:none;">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
          <button class="btn btn-success" id="approve-btn" type="button" title="Guardar como exemplo aprovado">Aprovar</button>
          <button class="btn btn-outline-success" id="thumb-up-btn" type="button" title="Gosto (sem aprovar)">👍</button>
          <button class="btn btn-outline-danger"  id="thumb-down-btn" type="button" title="Não gostei (sem aprovar)">👎</button>
          ...
          <span id="vote-status" class="small text-muted ms-1"></span>
        </div>
      </div>

      <!-- Suggestions container (populated by assets/js/main.js) -->
      <div id="ai-suggestions" class="mt-3" style="display:none;"></div>
...
(function(){
  ...
  const modelSel = document.getElementById('model_override');
  const quick = document.getElementById('quick_mode');
  ...
  // Persist and reconcile model selection vs quick mode
  function applyModelQuickInterlock() {
    const v = (modelSel && modelSel.value) ? modelSel.value.trim() : '';
    if (quick) {
      quick.disabled = !!v; // disable quick if a specific model is chosen
      if (v) quick.checked = false;
    }
  }
  if (modelSel) {
    // load previous selection
    try {
      const saved = localStorage.getItem('chatmind.model_override');
      if (saved !== null) {
        for (const opt of modelSel.options) {
          if (opt.value === saved) { modelSel.value = saved; break; }
        }
      }
    } catch(e){}
    applyModelQuickInterlock();
    modelSel.addEventListener('change', () => {
      try { localStorage.setItem('chatmind.model_override', modelSel.value); } catch(e){}
      applyModelQuickInterlock();
    });
  }
